<template>
  <div class="row g-0 border rounded overflow-hidden flex-md-row mb-2 shadow-sm h-md-250 position-relative">
    ...
    <div class="col p-4 d-flex flex-column position-static">
      ...
      <div class="mb-1 text-muted">{{ new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(cartItem.Product.price) }} x {{cartItem.amount}} pcs = {{ new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(cartItem.Product.price * cartItem.amount) }}</div>
    ...
    amount (newAmount, oldAmount) {
      if (newAmount > this.cartItem.Product.stock) {
        Swal.fire({
          icon: 'error',
          title: 'Oops',
          text: 'Amount can\'t exceed stock',
        ...
        this.amount = this.cartItem.Product.stock
      } else if (newAmount < 1) {
        Swal.fire({
          icon: 'error',
          title: 'Oops',
