<template>
  <div id="app">
    <ul class="nav nav-tabs justify-content-end">
      <li class="nav-item">
        <a class="nav-link active"><router-link to="/">Home</router-link></a>
      </li>
      <li class="nav-item">
        <a v-if="getAccesstoken" class="nav-link active"><router-link to="/cart">Cart</router-link></a>
      </li>
      <li class="nav-item">
        <a v-if="getAccesstoken" class="nav-link active"><router-link to="/wishlist">Wishlist</router-link></a>
      </li>
      <li class="nav-item">
        <a v-if="getAccesstoken" class="nav-link active"><router-link to="/history">History</router-link></a>
      </li>
      <li class="nav-item">
        <a v-if="!getAccesstoken" class="nav-link active"><router-link to="/login">Login</router-link></a>
      </li>
      <li class="nav-item">
        <a v-if="!getAccesstoken" class="nav-link active"><router-link to="/register">Register</router-link></a>
      </li>
      <li class="nav-item">
        <a v-if="getAccesstoken" @click="logout()" class="nav-link active"><router-link to="">Logout</router-link></a>
      </li>
      <li class="nav-item">
        <a class="nav-link active"><router-link to="/about">About</router-link></a>
      </li>
    </ul>
    <router-view/>
  </div>
</template>

<script>
export default {
  methods: {
    logout () {
      localStorage.clear()
      this.$store.commit('logout')
      this.$router.push('/login')
    }
  },
  computed: {
    getAccesstoken () {
      if (this.$store.state.accesstoken) {
        localStorage.setItem('accesstoken', this.$store.state.accesstoken)
      }
      return this.$store.state.accesstoken
    }
  },
  created: function () {
    this.$store.commit('accesstoken', localStorage.getItem('accesstoken'))
  }
}
</script>

<style>
body {
  background: #d0e2f5;
}

#app {
  font-family: Avenir, Helvetica, Arial, sans-serif;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
  text-align: center;
  color: #2c3e50;
}

#nav {
  padding: 30px;
}

#nav a {
  font-weight: bold;
  color: #2c3e50;
}

#nav a.router-link-exact-active {
  color: #42b983;
}

button {
  width: 150px;
}
</style>
